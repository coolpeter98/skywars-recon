...
- **Implemented server layers:** canonical remote resolution/routing, initial profile/party replication, character spawning, MemoryStore matchmaking, same-place reserved-server teleport/context, and an authoritative inventory service are present. Inventory now covers initial replication, item grants/removals, client prediction acknowledgement, held tools, armour, world drops/pickups, chest transfers, item/upgrade shop purchases, game-currency derivation, and minimal predicted-block reconciliation. Remaining priorities are: game-loop/map essentials; `GameSettings`/team/player replication; full blocks/chunks and breaking (`Workspace.BlockContainer`); combat authority; richer lobby services; party/private-match behavior; developer-product purchases (`MarketplaceService.ProcessReceipt`, gems ledger, `Profile.ProcessedReceipts`); persistence; and FX events.
...
- **Open / planned:**
  ...
  - **Domain services:** matchmaking, session plumbing, and inventory are present, while parties, persistence, developer-product purchases, combat, full game rounds/maps, team upgrades, and private-session behavior remain intentionally incomplete extension points for the user (per §3.8 priority list).
...
- **"Is the game playable right now?"** — The lobby is playable locally: the server foundation provides profile replication + character spawn, so the client boots to a fully interactable lobby UI (play button, party section, settings, shop/missions/locker, sidebar). Matchmaking and the inventory/drop/chest/item-shop foundation are implemented; full rounds/maps, combat, persistence, developer-product purchases, parties, and other domain behavior still need implementation (§5).
