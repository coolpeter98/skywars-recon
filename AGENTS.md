...
- **Lobby mapping + topbar:** unsaved Studio places (PlaceId 0) are treated as the lobby (documented Studio-only fallback in `place-util.isLobbyServer`); TopbarPlus icons never furl in Studio and are nudged into the viewport (Studio doesn't apply the topbar inset to ScreenGuis), making the settings gear clickable.
