...
- **Git repo:** `skywars-project/` is the git root (branch `main`), remote `origin` = `https://github.com/coolpeter98/skywars-decompile.git`. The workspace root `/home/peter/Documents/skywars-decompile/` also contains helper scripts that are **not** in the repo (see §9).
...
- **`ServerStorage/` is empty; there is no `ServerScriptService` at all.** The client-side save (FilteringEnabled) made server scripts impossible to save (the decompiler's own `src/README.server.luau` says so verbatim).
...
- **Done:** full rename + types on 734 scripts; named event access; everything committed and pushed to `origin/main`.
- **Open / planned (not started):**
  ...
  - **No server code exists.** Writing a minimal server (see §3.8 priority list) is the big open task.
  - User is setting up a **Rojo live session** + MCP bridge to Roblox Studio for runtime testing; expect runtime errors to be reported and debugged.
...
2. **Never touch `rbxts_include/`** — third-party compiled libraries.
...
4. **Keep behavior identical** in any reconstruction: logic, control flow, string literals, numbers, require/import paths, Roblox API calls, Instance/attribute names. Only identifiers, comments, and (additive) type annotations may change.
...
8. **Verify before/after editing:** `stylua --check <file>` must not report a parse error (formatting diffs alone are fine); `grep -nE '\b(v|p)[0-9]+\b|\b(v|p)_u_[0-9]+\b' <file>` must return nothing after a rename pass.
...
- Workspace root: `/home/peter/Documents/skywars-decompile/` (session working directory).
- Git repo root: `/home/peter/Documents/skywars-decompile/skywars-project/`, branch `main`, remote `origin` (GitHub `coolpeter98/skywars-decompile`). Commit+push via `git -c user.name="skywars-recon" -c user.email="recon@local" commit …` has been used; normal `git` identity also fine.
- `stylua` is available for parse checks/formatting (supports Luau types).
...
- **"Is the game playable right now?"** — Not without a server: the client hangs at remote creation / Profile replication / character spawn. A minimal server per §3.8 is required.
