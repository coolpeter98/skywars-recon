...
- **Mining merged geometry** (`mineCell` → `splitPart`): a single-cell part is destroyed outright; a merged part is replaced with **up to six axis-aligned boxes** (the standard box-minus-cell decomposition in `server/world/BlockGeometry.luau` — pure Luau, unit-tested under Lune by `tools/test_block_geometry.luau`, 858 checks). Face re-culling per new box (`describeFaces`): a face is textured when it **borders the mined cell** (the new hole) or when it sits on the **old part's exterior and was textured there**; fresh interior cut planes stay untextured — mirroring the map's culling, including the original's full-face overdraw where a merged face is only partially exposed. New boxes copy the old part's name/material/color/transparency/surfaces/physics and parent; texture ids come from the registry (fallback: any face of the material). **Neighbour re-culling** (`exposeNeighbourFaces`): faces of other parts that the mined cell used to hide get their texture re-added (add-if-missing, tiled per block, offset 0 — merged faces are cell-aligned so the pattern stays on the global grid). Partially-covered faces overdraw exactly like the shipped map, so visual style is preserved.
