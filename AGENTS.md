...
- **Placeholder maps:** `src/ServerStorage/Maps/SkyWars/<GameMode>/MapConfig.luau` contains one configuration per SkyWars mode. `PlaceholderMapService.luau` builds a reusable template with one island per team, a center island, team spawns, and loot chests. Chests clone the recovered `ReplicatedStorage/Misc/ChestTierOne..Four` models (real mesh/texture + their baked `AnimationController`/`Animator` rig, which the server drives — see the inventory bullet); island chests roll tiers 1-2 and center chests tiers 3-4 from `server/world/ChestLoot.luau` (bronze/iron gear and blocks on islands; gold/onyx gear, the bow, all six projectile weapons, explosives and the teleporter at center). Every existing client map alias remains selectable and is intentionally redirected to that mode's placeholder template until real models are supplied.
