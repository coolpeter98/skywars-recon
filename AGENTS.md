...
### 3.1 Multi-place topology

- The game is split across places: **Lobby** + game places (SkyWars, Duels, EggWars, Private), all enumerated in `ReplicatedStorage/TS/place/place-util.luau` (place groups with `Lobby.PlaceId`, `Games[]`, `Production` flags, testing variants).
- The saved place is the **Lobby** (`PlaceId 8542259458`). `PlaceUtil.isLobbyServer()/isGameServer()/getGameMode()` branch client behavior on `game.PlaceId` — running at a different PlaceId changes behavior (auto-queue only runs on the lobby place).
...
- **`ServerStorage/` is empty; the original place had no `ServerScriptService`.** The client-side save (FilteringEnabled) made server scripts impossible to save (the decompiler's own `src/README.server.luau` says so verbatim). A minimal stub now lives at `src/ServerScriptService/MinimalServer.server.luau` (§4.4).
...
- **What a server must implement** (prioritized for "starts without erroring"): (1) Flamework `createServer` over the same `TS/events.luau` schema → creates all remotes with `id` attributes; (2) on join: `SetValue` `@@INIT` + `Profile` (+ `Party`, `GameState`, `GameTeams`, `GameCurrency`, `Players`, `GameSettings`); (3) spawn characters; (4) lobby world (`Events.JoinWorld` + `Workspace.Lobby`); (5) matchmaking handshake; (6) game-loop essentials; (7) blocks/chunks (`Workspace.BlockContainer`); (8) inventory/combat authority; (9) lobby services; (10) party/private-match; (11) purchases (`MarketplaceService.ProcessReceipt` for the 15 gamepass ids + dev products, gems ledger, `Profile.ProcessedReceipts`); (12) teleport; (13) FX events (fire-and-forget).
...
- **Lobby mapping + topbar:** unsaved Studio places (PlaceId 0) are treated as the lobby (documented Studio-only fallback in `place-util.isLobbyServer`); TopbarPlus icons never furl in Studio and their shared holder is nudged into the viewport exactly once (Studio doesn't apply the topbar inset to ScreenGuis), making the settings gear clickable without pushing multiple icons too far down.
- **Minimal server stub:** `src/ServerScriptService/MinimalServer.server.luau` (mapped in `default.project.json`) sends `SetValue` `@@INIT` + `SharedDataProfileTemplate` (retrying until the client acks via `LoginStreak`), answers `GetCustomServers` with an empty list via the Flamework function protocol, and `LoadCharacter`s on join. Deliberately no persistence/purchases/parties/matchmaking — future server work.
...
- **Done:** full rename + types on 734 scripts; named event access; **the client now boots error-free in Studio and shows a fully interactable lobby UI** (play button → queue screen, party section, settings gear → settings menu, shop/missions/locker buttons, profile sidebar), with character spawn + profile replication provided by `MinimalServer` (§4.4). All work committed to `main`.
- **Open / planned:**
  ...
  - **Real server:** `MinimalServer` is a stub. Real matchmaking, parties, persistence, purchases (per §3.8 priority list) remain to be written; the stub is designed to be replaced wholesale.
...
| `ReplicatedStorage/TS/place/place-util.luau` | Place groups / lobby-vs-game detection |
...
- **"Is the game playable right now?"** — The lobby is playable locally: with `MinimalServer` (profile replication + character spawn) the client boots to a fully interactable lobby UI (play button, party section, settings, shop/missions/locker, sidebar). Real matchmaking/persistence/purchases need the real server (§5).
