...
- **Lobby mapping + topbar:** unsaved Studio places (PlaceId 0) are treated as the lobby (documented Studio-only fallback in `place-util.isLobbyServer`); TopbarPlus icons never furl in Studio, keeping the settings gear visible and clickable while preserving TopbarPlus's native alignment with the Roblox topbar.
